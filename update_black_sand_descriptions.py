...
# -*- coding: utf-8 -*-
"""
Script to update Black Sand Brewery content in all description master files
"""
import json
import os

def update_black_sand_descriptions():
    """Update Black Sand Brewery content in all language description master files"""
    
    # Define the content for each language (same as before)
    content_updates = {
        'zh': [
            {
                "title": "概念与氛围",
    ...
    # Base path for description files
    ...
    for lang, sections in content_updates.items():
        ...
        try:
            ...
            # Find Black Sand Brewery entry by placeId
            place_id = 'ChIJNzjyIBI50i0RpFdnd_ZN3pg'
            if place_id in data and data[place_id].get('name') == 'Black Sand Brewery':
                print(f"  Found Black Sand Brewery with placeId: {place_id}")
                print(f"  Current sections: {len(data[place_id].get('sections', []))}")
                
                # Update the sections
                data[place_id]['sections'] = sections
    ...
    print("\n✨ Description files update completed!")

if __name__ == "__main__":
    update_black_sand_descriptions()